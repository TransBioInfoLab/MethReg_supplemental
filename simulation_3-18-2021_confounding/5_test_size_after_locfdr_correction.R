...
for (i in 1:9){
  ...
  idx <- complete.cases(res.both)

  res.both <- res.both[idx, ]
  
  res.both$zvalue <- qnorm(pt(res.both$tValue, res.both$df))
  
  res.both <- subset (res.both, is.finite(res.both$zvalue))  ## added 3-16-21
                                                             ## to remove inf values in zvalue
  ...
  one.test.size <- data.frame (beta.tf = i,
                               test.size = test.size)
...
all <- rbind (all.test.size, c("avg", mean(all.test.size$test.size)))

saveRDS (all, "./results/rlmBinary.en_all_test_size.RDS")
